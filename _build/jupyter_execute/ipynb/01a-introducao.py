...
# - _Maximização da razão "tinta-dados": para Tufte, o _storyteller_ precisa atentar para a quantidade de elementos agregados em uma representação visual. Sobrecarregar a interpretação do leitor com elementos geométricos, bordas, cores de fundo, efeitos 3D, entre outros adereços, é um ponto negativo para a compreensão final. A maneira sugerida de controlar a superfluidade visual é calcular a quantidade de tinta necessária para representar os dados reais sem ambiguidade e comparar com a quantidade de tinta utilizada para enriquecer a figura com decorações. A equação para o que Tufte chamou de razão "tinta-dados"_ (_data-ink ratio_), aqui denotada por $\text{RTD}$ é: 
# 
# $$\text{RTD} = \dfrac{d - t}{T},$$
# 
